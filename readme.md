...
回复首行包含不可见占位符，用于确保 QQ 自动添加的用户提及与标题分行显示。
...
- 三个签到命令统一增加不可见的非空首行，避免 QQ 自动用户提及与标题显示在同一行

## 1.2.3 更新说明

- 最近签到改为按签到日期排序，避免迁移的历史记录因自增 ID 较大而排在前面

## 1.2.2 更新说明

- `getTotal` 和 `getRecent` 恢复与签到回复一致的分段样式
- 最近签到继续展示运势点数，并保留无记录提示

## 1.2.1 更新说明

- 恢复分段式签到回复，区分标题、结果和签到限制
- 在回复开头保留空行，避免标题与用户提及显示在同一行
